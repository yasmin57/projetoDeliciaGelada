<!DOCTYPE html>
<html lang="pt-br">
    <head>
        <title>
            Promoções| Delicia Gelada
        </title>
       <link href="../css/style.css" type="text/css" rel="stylesheet">
       <link href="../css/promocoes.css" type="text/css" rel="stylesheet">
        <link href="https://fonts.googleapis.com/css?family=Fjalla+One&display=swap" rel="stylesheet">
    </head>
    <body>
       <!-- CABEÇALHO -->
        <?php require_once("header.php"); ?>
        
        <!-- CONTEUDO -->
        <div id="promocoes">
            <!--  Slogan     -->
            <div class="back_goiaba" id="faixa_slogan">
                <div class="conteudo back_goiaba center ">
                    <table class="promocoes_table">
                        <tr>
                            <td class="fonte" id="slogan">
                                O melhor das
                                frutas e vegetais,
                                <div class="back_orange" id="slogan_right"> no melhor dos sucos</div>
                                <div id="slogan_left" class="back_pink"> com o melhor preço</div>
                            </td>
                        </tr>
                    </table>
                </div>
            </div>
            <!-- faixa 01  -->
            <div class="promocoes_faixa">
                <div class="conteudo center">
                    <table class="promocoes_table">
                        <tr>
                            <td class="promocoes_img back">
                                <img src="../imgs/promocoes_img1.png" alt="imagem">
                            </td>
                            <td class="promocoes_txt fonte">
                                <h1> Del Valle Frut </h1>
                                <p> Descrição: Del Valle Frut traz para as refeições em família as cores vibrantes e o sabor das frutas. As novas receitas são deliciosas, têm 60% menos açúcares que a média do mercado e são fonte de vitamina C. Contém 1 litro. </p>
                                <div class="preco_antigo back_orange"> De: R$ 6,64 </div>
                                <div class="preco_atual back_pink float"> Por: R$ 5,58 </div>
                                <div class="btn_compre back_green_limao_light float"> Compre </div>
                            </td>
                            
                            <td class="promocoes_img back">
                                <img src="../imgs/promocoes_img1.png" alt="imagem">
                            </td>
                            <td class="promocoes_txt fonte">
                                <h1> Del Valle Frut </h1>
                                <p> Descrição: Del Valle Frut traz para as refeições em família as cores vibrantes e o sabor das frutas. As novas receitas são deliciosas, têm 60% menos açúcares que a média do mercado e são fonte de vitamina C. Contém 1 litro. </p>
                                <div class="preco_antigo back_orange"> De: R$ 6,64 </div>
                                <div class="preco_atual back_pink float"> Por: R$ 5,58 </div>
                                <div class="btn_compre back_green_limao_light float"> Compre </div>
                            </td>
                        </tr>
                    </table>
                </div>
            </div>
            <!-- faixa 02  -->
            <div class="promocoes_faixa">
                <div class="conteudo center">
                    <table class="promocoes_table">
                        <tr>
                            <td class="promocoes_img back">
                                <img src="../imgs/promocoes_img1.png" alt="imagem">
                            </td>
                            <td class="promocoes_txt fonte">
                                <h1> Del Valle Frut </h1>
                                <p> Descrição: Del Valle Frut traz para as refeições em família as cores vibrantes e o sabor das frutas. As novas receitas são deliciosas, têm 60% menos açúcares que a média do mercado e são fonte de vitamina C. Contém 1 litro. </p>
                                <div class="preco_antigo back_orange"> De: R$ 6,64 </div>
                                <div class="preco_atual back_pink float"> Por: R$ 5,58 </div>
                                <div class="btn_compre back_green_limao_light float"> Compre </div>
                            </td>
                            
                            <td class="promocoes_img back">
                                <img src="../imgs/promocoes_img1.png" alt="imagem">
                            </td>
                            <td class="promocoes_txt fonte">
                                <h1> Del Valle Frut </h1>
                                <p> Descrição: Del Valle Frut traz para as refeições em família as cores vibrantes e o sabor das frutas. As novas receitas são deliciosas, têm 60% menos açúcares que a média do mercado e são fonte de vitamina C. Contém 1 litro. </p>
                                <div class="preco_antigo back_orange"> De: R$ 6,64 </div>
                                <div class="preco_atual back_pink float"> Por: R$ 5,58 </div>
                                <div class="btn_compre back_green_limao_light float"> Compre </div>
                            </td>
                        </tr>
                    </table>
                </div>
            </div>
        </div>
        
        <!-- RODAPÉ  -->
       <?php require_once("footer.php"); ?>
    </body>
</html>